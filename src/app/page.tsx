...
function Children({kids,members,onSelect,political=false}:{
  ...
  const border = political ? `2px dashed ${color}` : `3px solid ${color}`
  if(kids.length===1) return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
      <VLine h={20} color={color}/>
      <TreeNode person={kids[0]} members={members} onSelect={onSelect}/>
    </div>
  )
  return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
    ...
    <div style={{display:'flex',alignItems:'flex-start',gap:0}}>
      ...
      {prev.spouse&&(
        <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
          ...
          <Children kids={prev.spouseOwnChildren} members={members} onSelect={onSelect} political={true}/>
        </div>
      )}
      ...
      {curr.spouse&&(
        <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
          ...
          <Children kids={curr.children} members={members} onSelect={onSelect}/>
          <Children kids={curr.spouseOwnChildren} members={members} onSelect={onSelect} political={true}/>
        </div>
      )}
    </div>
  )
...
// [Left]——[Right] with kids hanging from join point center
function Pair({left,right,kids,members,onSelect,political=false}:{
  left:Member,right:Member|null,kids:Member[],
  members:Member[],onSelect:(p:Member)=>void,political?:boolean
}){
  return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
      <div style={{display:'flex',alignItems:'center'}}>
      ...
      <Kids list={kids} members={members} onSelect={onSelect} political={political}/>
    </div>
  )
}

// Spouse with own unknown-father kids
function SpouseWithOwnKids({spouse,ownKids,members,onSelect}:{
...
function TreeNode({person,members,onSelect}:{person:Member;members:Member[];onSelect:(p:Member)=>void}){
  ...
  if(marriages.length===1){
    ...
    if(spouseOwnChildren.length>0 && spouse){
      ...
      const spLeft = left===spouse
      return (
        <div style={{display:'flex',alignItems:'flex-start',gap:0}}>
          {spLeft ? <>
            <SpouseWithOwnKids spouse={spouse} ownKids={spouseOwnChildren} members={members} onSelect={onSelect}/>
  ...
  // ── MULTIPLE MARRIAGES ────────────────────────────────────────────
  // TWO Pair blocks sharing Person:
  // Pair1: [PrevSpouse]——[Person]  → prevKids centered below join ✓
  // Pair2: [Person]——[CurrSpouse]  → currKids centered below join ✓
  //
  // Person is rendered TWICE (once in each Pair) side by side.
  // This is the only way to guarantee kids hang from the correct join point.
  // The two Person cards appear adjacent, connected by the marriage lines.

  const prev = marriages[0]
  const curr = marriages[marriages.length-1]

  // Determine order within each pair (M left, F right)
  const p1left  = prev.spouse?.gender==='M' ? prev.spouse : person
  const p1right = p1left===person ? prev.spouse : person

  const p2left  = person.gender==='M' ? person : curr.spouse
  const p2right = p2left===person ? curr.spouse : person

  return (
    <div style={{display:'flex',alignItems:'flex-start',gap:0}}>

      {/* PAIR 1: prev spouse + person, prevKids below */}
      {prev.spouse ? (
        prev.spouseOwnChildren.length>0 ? (
          // Prev spouse has own kids
          <div style={{display:'flex',alignItems:'flex-start',gap:0}}>
            <SpouseWithOwnKids spouse={prev.spouse} ownKids={prev.spouseOwnChildren} members={members} onSelect={onSelect}/>
            <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
              <div style={{display:'flex',alignItems:'center'}}>
                <div style={{width:20,height:3,background:MARRY_COLOR,flexShrink:0}}/>
                <MiniCard person={person} onSelect={onSelect}/>
          ...
          <Pair left={p1left} right={p1right??null} kids={prev.children} members={members} onSelect={onSelect}/>
        )
      ) : (
        <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
          <MiniCard person={person} onSelect={onSelect}/>
      ...
      {/* PAIR 2: person + curr spouse, currKids below */}
      {curr.spouse&&(
        curr.spouseOwnChildren.length>0 ? (
          <div style={{display:'flex',alignItems:'flex-start',gap:0}}>
            ...
            <div style={{width:20,height:3,background:MARRY_COLOR,alignSelf:'flex-start',marginTop:28,flexShrink:0}}/>
            <SpouseWithOwnKids spouse={curr.spouse} ownKids={curr.spouseOwnChildren} members={members} onSelect={onSelect}/>
          </div>
        ) : (
          <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
            <div style={{display:'flex',alignItems:'center'}}>
...
// Renders [Left]——[Right] couple with kids hanging from the JOIN POINT (center of marriage line)
// This is THE fundamental building block - kids ALWAYS come from the couple center
function Pair({left,right,kids,members,onSelect,political=false}:{
  left:Member,right:Member|null,kids:Member[],
  members:Member[],onSelect:(p:Member)=>void,political?:boolean
}){
...
function TreeNode({person,members,onSelect}:{person:Member;members:Member[];onSelect:(p:Member)=>void}){
  ...
  if(marriages.length===1){
        ...
        <div style={{display:'flex',alignItems:'flex-start',gap:0}}>
          {spLeft ? <>
            <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
              ...
              <Kids list={spouseOwnChildren} members={members} onSelect={onSelect} political={true}/>
            </div>
            <div style={{width:20,height:3,background:MARRY_COLOR,alignSelf:'flex-start',marginTop:28,flexShrink:0}}/>
            <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
              <MiniCard person={person} onSelect={onSelect}/>
    ...
    // Normal: Pair ensures kids hang from couple center
    return <Pair left={left} right={right} kids={children} members={members} onSelect={onSelect}/>
  }

  // ── MULTIPLE MARRIAGES ────────────────────────────────────────────
  // We render TWO Pair blocks:
  // Block1: [PrevSpouse]——[Person] with prevKids below the JOIN
  // Block2: [Person]——[CurrSpouse] with currKids below the JOIN
  //
  // BUT Person appears in both! We render it only in Block1,
  // and Block2 shows just CurrSpouse with its own column.
  // The marriage line from Person to CurrSpouse is shown in Block1's row.
  //
  // This gives us:
  // [PrevSpouse]——[Person]——(line)  [CurrSpouse]
  //       prevKids↑                  currKids↑
  //
  // Block1 is centered, so prevKids center under [PrevSpouse——Person——line]
  // Block2 currKids center under [CurrSpouse]
  //
  // Not perfect but much better than kids hanging from person alone.

  const prev = marriages[0]
  const curr = marriages[marriages.length-1]

  // For Block1: left=PrevSpouse (or person if no prev spouse), right=person
  // M always left, F always right within each pair
  const prevLeft  = prev.spouse
    ? (prev.spouse.gender==='M' ? prev.spouse : person)
    : person
  const prevRight = prev.spouse
    ? (prev.spouse.gender==='M' ? person : prev.spouse)
    : null

  return (
    <div style={{display:'flex',alignItems:'flex-start',gap:0}}>

      {/* BLOCK 1: prev couple + prevKids */}
      {prev.spouse&&prev.spouseOwnChildren.length>0 ? (
        // Prev spouse has own kids - show them in a column
        <div style={{display:'flex',alignItems:'flex-start',gap:0}}>
          <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
            <MiniCard person={prev.spouse} onSelect={onSelect}/>
            <VLine h={14} color={POLIT_COLOR}/>
        ...
        // Normal prev couple - use Pair so prevKids hang from join
        <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
          <div style={{display:'flex',alignItems:'center'}}>
            <MiniCard person={prevLeft} onSelect={onSelect}/>
            {prev.spouse&&<div style={{width:20,height:3,background:MARRY_COLOR,flexShrink:0}}/>}
            {prev.spouse&&<MiniCard person={prevRight!} onSelect={onSelect}/>}
            {curr.spouse&&<div style={{width:20,height:3,background:MARRY_COLOR,flexShrink:0}}/>}
          </div>
          <Kids list={prev.children} members={members} onSelect={onSelect}/>
        </div>
      ...
      {/* BLOCK 2: curr spouse column + currKids */}
      {curr.spouse&&(
        <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
          <MiniCard person={curr.spouse} onSelect={onSelect}/>
          <Kids list={curr.children} members={members} onSelect={onSelect}/>
          {curr.spouseOwnChildren.length>0&&<>
            ...
            <Kids list={curr.spouseOwnChildren} members={members} onSelect={onSelect} political={true}/>
          </>}
        </div>
      )}
    </div>
...
function UnknownParent(){
  return (
    <div style={{
      width:60,height:60,borderRadius:12,border:`2px dashed #94a3b8`,
      background:'#f8fafc',display:'flex',flexDirection:'column',
      alignItems:'center',justifyContent:'center',
      color:'#94a3b8',fontSize:20,fontWeight:700,flexShrink:0
    }}>
      <div>?</div>
      <div style={{fontSize:9,marginTop:2}}>no registrado</div>
    </div>
  )
}

// Children hanging from a join point above
// Horizontal bar spans exactly center-to-center
function Kids({list,members,onSelect,political=false}:{
  list:Member[],members:Member[],onSelect:(p:Member)=>void,political?:boolean
}){
  if(!list.length) return null
...
// THE KEY COMPONENT: renders a couple side-by-side and their kids
// hanging from the CENTER of the marriage line (join point)
// This guarantees kids always connect from between the two parents
function Pair({left,right,kids,members,onSelect,political=false}:{
  left:Member, right:Member|null,
  kids:Member[], members:Member[],
  onSelect:(p:Member)=>void, political?:boolean
}){
  return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
      <div style={{display:'flex',alignItems:'center'}}>
...
function TreeNode({person,members,onSelect}:{person:Member;members:Member[];onSelect:(p:Member)=>void}){
  ...
  if(marriages.length===1){
    ...
    // If spouse has own kids with unknown other parent
    if(spouseOwnChildren.length>0 && spouse){
      const spouseLeft = spouse.gender==='M'
      return (
        <div style={{display:'flex',alignItems:'flex-start',gap:0}}>
          {spouseLeft ? <>
            {/* Spouse LEFT with unknown parent + own kids */}
            <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
              <Pair left={spouse} right={null} kids={[]} members={members} onSelect={onSelect}/>
              <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
                <VLine h={16} color={POLIT_COLOR}/>
                <div style={{display:'flex',alignItems:'center'}}>
                  <div style={{width:20,height:2,borderTop:`2px dashed ${POLIT_COLOR}`}}/>
                  <UnknownParent/>
                </div>
            ...
            <div style={{width:20,height:3,background:MARRY_COLOR,alignSelf:'flex-start',marginTop:28,flexShrink:0}}/>
            <Pair left={person} right={null} kids={children} members={members} onSelect={onSelect}/>
          </> : <>
            {/* Person LEFT, spouse RIGHT with unknown parent + own kids */}
            <Pair left={person} right={null} kids={children} members={members} onSelect={onSelect}/>
            <div style={{width:20,height:3,background:MARRY_COLOR,alignSelf:'flex-start',marginTop:28,flexShrink:0}}/>
            <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
              <MiniCard person={spouse} onSelect={onSelect}/>
    ...
    // Normal: use Pair so kids hang from couple center
    return <Pair left={left} right={right} kids={children} members={members} onSelect={onSelect}/>
  }
  ...
  //
  // CRITICAL: prevKids must hang from the JOIN between PrevSpouse and Person
  //           currKids must hang from the JOIN between Person and CurrSpouse
  //
  // We achieve this by rendering TWO Pair components side by side:
  // Pair1: [PrevSpouse]——[Person] with prevKids below (centered under join)
  // Pair2: [Person]——[CurrSpouse] with currKids below (centered under join)
  // BUT Person appears twice! We use a trick: render Person only in Pair1,
  // and for Pair2 we just show CurrSpouse with a connector line from Person
  //
  // Actually: we render the whole thing as one horizontal block:
  // [PrevSpouseCol] [MarryLine] [PersonCol+prevKids] [MarryLine] [CurrSpouseCol+currKids]
  // Where PersonCol is centered between the two marry lines
  // prevKids center = center of (PrevSpouse + MarryLine + Person) = slightly right of line
  // This is the best we can do without absolute positioning

  const prev = marriages[0]
  const curr = marriages[marriages.length-1]

  // Use Pair for each marriage, sharing Person as the right of prev and left of curr
  // prev pair: [PrevSpouse]——[Person] + prevKids
  // curr pair: [Person]——[CurrSpouse] + currKids  ← but Person is already shown!
  // Solution: show prev pair normally, then append curr spouse to the right

  // Render as three sections:
  // 1. PrevSpouse col (with own kids if any)
  // 2. Center block: [PrevSpouse——Person——CurrSpouse] row
  //    - prevKids hang from center of [PrevSpouse——Person]
  //    - currKids hang from center of [Person——CurrSpouse]  
  //    We achieve this by centering over the full couple row
  //    and accepting slight offset for prevKids vs currKids
  // 3. CurrSpouse own kids (political)
  ...
  return (
    <div style={{display:'flex',alignItems:'flex-start',gap:0}}>
      ...
      {/* CENTER: [——Person——] with prevKids below */}
      <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
        <div style={{display:'flex',alignItems:'center'}}>
          {prev.spouse&&<div style={{width:20,height:3,background:MARRY_COLOR,flexShrink:0}}/>}
          <MiniCard person={person} onSelect={onSelect}/>
      ...
      {/* RIGHT: CurrSpouse col with currKids + own kids */}
      {curr.spouse&&(
        <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
          <MiniCard person={curr.spouse} onSelect={onSelect}/>
          <Kids list={curr.children} members={members} onSelect={onSelect}/>
...
// Unknown parent symbol - shown when father/mother is not registered
function UnknownParent({onSelect}:{onSelect:()=>void}){
  return (
        ...
        {kids.map((kid,i)=>(
          <div key={kid.id} style={{display:'flex',flexDirection:'column',alignItems:'center',margin:'0 10px'}}>
            <div style={{display:'flex',alignItems:'center',width:'100%'}}>
              <div style={{flex:1,height:3,background:i===0?'transparent':color}}/>
              <VLine h={20} color={color}/>
              <div style={{flex:1,height:3,background:i===kids.length-1?'transparent':color}}/>
            </div>
            <TreeNode person={kid} members={members} onSelect={onSelect}/>
          </div>
        ))}
...
// A couple: [Left]——[Right] with their shared kids hanging from the JOIN POINT
// The join point is the center of the marriage line between the two cards
function CoupleBlock({left,right,kids,members,onSelect}:{
  left:Member, right:Member|null, kids:Member[],
  members:Member[], onSelect:(p:Member)=>void
}){
  if(!right){
  ...
  // Couple side by side, kids hang from center (between the two cards)
  return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
      <div style={{display:'flex',alignItems:'center'}}>
        <MiniCard person={left} onSelect={onSelect}/>
...
// Political parent column: card + unknown-parent symbol + kids
// Used for Angélica (Convalía kids) and Tere Vial (Mascaró kids)
function PoliticalParentWithKids({parent,kids,members,onSelect}:{
  parent:Member, kids:Member[], members:Member[], onSelect:(p:Member)=>void
}){
  if(!kids.length) return <MiniCard person={parent} onSelect={onSelect}/>
  return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
      <MiniCard person={parent} onSelect={onSelect}/>
      {/* Unknown parent symbol */}
      <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
        <VLine h={16} color={POLIT_COLOR}/>
        <div style={{display:'flex',alignItems:'center'}}>
          <div style={{width:20,height:2,borderTop:`2px dashed ${POLIT_COLOR}`}}/>
          <UnknownParent onSelect={()=>{}}/>
        </div>
      </div>
      {/* Kids hang from the join point (between parent and unknown) */}
      <Children kids={kids} members={members} onSelect={onSelect} political={true}/>
    </div>
  )
}
...
function TreeNode({person,members,onSelect}:{person:Member;members:Member[];onSelect:(p:Member)=>void}){
  ...
  if(marriages.length===1){
    ...
    // If spouse has own children (e.g. Angélica): show them with unknown-parent symbol
    if(spouseOwnChildren.length > 0 && spouse){
      ...
      return (
        <div style={{display:'flex',alignItems:'flex-start',gap:0}}>
          {spouseIsLeft ? (
            <>
              <PoliticalParentWithKids parent={spouse} kids={spouseOwnChildren} members={members} onSelect={onSelect}/>
              <div style={{width:20,height:3,background:MARRY_COLOR,alignSelf:'flex-start',marginTop:28,flexShrink:0}}/>
              <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
                <MiniCard person={person} onSelect={onSelect}/>
                <Children kids={children} members={members} onSelect={onSelect}/>
              </div>
            </>
          ) : (
            <>
    ...
    // Normal single marriage
    const left  = !spouse||person.gender==='M' ? person : spouse
    const right = left===person ? spouse : person
    return <CoupleBlock left={left} right={right??null} kids={children} members={members} onSelect={onSelect}/>
  }
  ...
  //                               [ownKids]
  //
  // KEY: prevKids hang from JOIN between PrevSpouse and Person
  //      currKids hang from JOIN between Person and CurrSpouse
  //      Each JOIN is represented by the CoupleBlock component
  ...
  return (
    <div style={{display:'flex',alignItems:'flex-start',gap:0}}>
      ...
      {/* LEFT: PrevSpouse with own kids (using PoliticalParentWithKids if has own kids) */}
      {prev.spouse&&(
        <>
          {prev.spouseOwnChildren.length>0 ? (
            <PoliticalParentWithKids parent={prev.spouse} kids={prev.spouseOwnChildren} members={members} onSelect={onSelect}/>
          ) : (
            <MiniCard person={prev.spouse} onSelect={onSelect}/>
          )}
          <div style={{width:20,height:3,background:MARRY_COLOR,alignSelf:'flex-start',marginTop:28,flexShrink:0}}/>
      ...
      {/* CENTER: Person with prevKids hanging below (from the join point) */}
      <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
        <MiniCard person={person} onSelect={onSelect}/>
        <Children kids={prev.children} members={members} onSelect={onSelect}/>
      </div>

      {/* RIGHT: CurrSpouse with currKids + own kids */}
      {curr.spouse&&(
        <>
          <div style={{width:20,height:3,background:MARRY_COLOR,alignSelf:'flex-start',marginTop:28,flexShrink:0}}/>
          {curr.spouseOwnChildren.length>0 ? (
            <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
              <MiniCard person={curr.spouse} onSelect={onSelect}/>
              <Children kids={curr.children} members={members} onSelect={onSelect}/>
              <PoliticalParentWithKids parent={curr.spouse} kids={curr.spouseOwnChildren} members={members} onSelect={onSelect}/>
            </div>
          ) : (
            <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
              <MiniCard person={curr.spouse} onSelect={onSelect}/>
