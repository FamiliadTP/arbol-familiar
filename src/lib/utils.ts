  ...
  return (
    <div style={{display:'flex', alignItems:'flex-start', gap:24}}>
      ...
      {prev.spouseOwnChildren.length > 0 && prev.spouse && (()=>{
        ...
        console.log('DEBUG bio_notes:', person.bio_notes)
        console.log('DEBUG prevMarriage:', prevMarriage)
        console.log('DEBUG spouse_own_partner_id:', prevMarriage?.spouse_own_partner_id)
        const ownPartner = prevMarriage?.spouse_own_partner_id
          ? members.find(m => m.id === prevMarriage.spouse_own_partner_id) ?? null
          : null
        console.log('DEBUG ownPartner:', ownPartner)
        return (
          <div style={{display:'flex', flexDirection:'column', alignItems:'center'}}>
            <div style={{display:'flex', alignItems:'center'}}>
              {ownPartner
